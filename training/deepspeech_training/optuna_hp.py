...
def create_model(n_hidden, batch_x, seq_length, dropout, reuse=False, batch_size=None, previous_state=None, overlap=True, rnn_impl=rnn_impl_lstmblockfusedcell):
    layers = {}
    ...
    layers['layer_1'] = layer_1 = dense('layer_1', batch_x, n_hidden, dropout_rate=dropout[0], layer_norm=FLAGS.layer_norm)
    layers['layer_2'] = layer_2 = dense('layer_2', layer_1, n_hidden, dropout_rate=dropout[1], layer_norm=FLAGS.layer_norm)
    layers['layer_3'] = layer_3 = dense('layer_3', layer_2, n_hidden, dropout_rate=dropout[2], layer_norm=FLAGS.layer_norm)

    # `layer_3` is now reshaped into `[n_steps, batch_size, 2*n_cell_dim]`,
    # as the LSTM RNN expects its input to be of shape `[max_time, batch_size, input_size]`.
    layer_3 = tf.reshape(layer_3, [-1, batch_size, n_hidden])
    ...
    layers['layer_5'] = layer_5 = dense('layer_5', output, n_hidden, dropout_rate=dropout[5], layer_norm=FLAGS.layer_norm)

    # Now we apply a final linear layer creating `n_classes` dimensional vectors, the logits.
    layers['layer_6'] = layer_6 = dense('layer_6', layer_5, 36, relu=False)
...
def calculate_mean_edit_distance_and_loss(n_hidden, iterator, dropout, reuse):
    r'''
    ...
    logits, _ = create_model(n_hidden, batch_x, batch_seq_len, dropout, reuse=reuse, rnn_impl=rnn_impl)
...
def get_tower_results(n_hidden, iterator, optimizer, dropout_rates):
    r'''
        ...
        for i in range(len(Config.available_devices)):
            ...
            with tf.device(device):
                ...
                with tf.name_scope('tower_%d' % i):
                    ...
                    avg_loss, non_finite_files = calculate_mean_edit_distance_and_loss(n_hidden, iterator, dropout_rates, reuse=i > 0)
...
def hps_create_optimizer(trial):
    learning_rate = trial.suggest_float("adam_lr", 1e-6, 1e-3, log=True)
    dropout_rate = trial.suggest_categorical("dropout_rate", [0.05, 0.1, 0.2, 0.25, 0.3, 0.4, 0.5])
    train_batch_size = trial.suggest_categorical("train_batch_size", [64, 128, 256, 512])
    n_hidden = trial.suggest_categorical("n_hidden", [1024, 2048, 3072, 4096])
    
    if n_hidden == 4096:
        train_batch_size = min(256, train_batch_size)  
    ...
    optimizer = tfv1.train.AdamOptimizer(learning_rate=learning_rate_var, beta1=0.9, beta2=0.999, epsilon=1e-08)
    return optimizer, learning_rate_var, dropout_rate, train_batch_size, n_hidden


def train(trial):
    exception_box = ExceptionBox()
    optimizer, learning_rate_var, dropout_rate, train_batch_size, n_hidden = hps_create_optimizer(trial)
    
    if (train_batch_size == 512) and (n_hidden == 4096):
        raise optuna.structs.TrialPruned()
    
    print(learning_rate_var, dropout_rate, train_batch_size, n_hidden)
    ...
    train_set = create_dataset(FLAGS.train_files.split(','),
                               ...
                               process_ahead=len(Config.available_devices) * train_batch_size * 2,
    ...
    gradients, loss, non_finite_files = get_tower_results(n_hidden, iterator, optimizer, dropout_rates)
...
def setup_dirs(study_name, trial_number):
    #(study.study_name, trial.number + 1)
    import os
    checkpoint_root_dir = "./checkpoints/"
    checkpoint_dir = os.path.join(checkpoint_root_dir, f"{study_name}_{os.environ['CUDA_VISIBLE_DEVICES']}_{trial_number}")
    return checkpoint_dir
    

def new_trial_callback(study, trial):
    trial_number = trial.number if trial is not None else 0
    chkpt_path = setup_dirs(study.study_name, trial_number)
    FLAGS.checkpoint_dir = chkpt_path 
    FLAGS.save_checkpoint_dir = chkpt_path 
    FLAGS.load_checkpoint_dir = chkpt_path 
    print(FLAGS.checkpoint_dir, FLAGS.save_checkpoint_dir, FLAGS.load_checkpoint_dir)


def objective(trial):
    if FLAGS.train_files:
        val_loss = train(trial)
    rnn_impl_cudnn_rnn.cell = None    
...
def main(_):
    ...
    study = optuna.create_study(study_name="deepspeech_optimization_v1.0", 
                                   direction='minimize', 
                                   pruner=optuna.pruners.MedianPruner(),
                                   load_exist=True,
                                   storage='sqlite:///example.db')
    new_trial_callback(study, trial=None)
    study.optimize(objective_tf, n_trials=10, callbacks=[new_trial_callback])
